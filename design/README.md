...
- Six interactive elements are **under 44px at 390** despite the token sheet claiming the minimum is never breached: grid cell 39, record window tab 40, settings sign-out 40, record year chip 28, footer link 24, banner dismiss ~22. See `export/layout.md`. **Note the bar:** WCAG 2.2 AA (SC 2.5.8) is 24×24, not 44 — 44 is AAA (SC 2.5.5) and Apple HIG. Only the banner dismiss actually breaches AA. Resolution in DECISIONS.md NONET-9: fix the dismiss, lift the 40px and 28px controls to 44, and accept 39px grid cells at 390 as a documented exception, since nine 44px cells do not fit a 390px viewport without full-bleed.
